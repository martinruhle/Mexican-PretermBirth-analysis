...
**Selection criteria**:
- Liberal significance threshold (p<0.30): Justified by small sample size (n~34 per training fold) to balance Type I versus Type II error risks. Stricter thresholds (p<0.05) would miss potentially important features due to limited statistical power.
...
- Top 10 selection: Among non-collinear variables meeting p<0.30, the top 10 by p-value rank were selected. This threshold balances information inclusion with dimensionality control—with only ~34 training subjects per fold, more than 10 features risks overfitting even with regularized models.
...
- CLR transformation: Compositional per-sample centered log-ratio — each sample (row) is closed by the geometric mean of its own taxa, applied within each fold. Because per-sample CLR is row-local it introduces no train→test leakage. Zeros are replaced first with unsupervised Bayesian-multiplicative estimates (zCompositions::cmultRepl), learned once on the full microbiome (outcome-blind)
...
**Data Structure** (dominios separados por nombre vía `role` en `config/data_dictionary.csv`, no por posición):
- **Microbiome**: Genus-level relative abundances (`role = microbiome`; 97 taxa)
- **Clinical**: Demographics, anthropometrics, complications (columnas no-taxa)
- **Keys / Outcomes**: `index` (sample_id), `id` (subject_id), `sdg_parto` (GA), `preterm` (binary)
...
```{r load_data}
# ----------------------------------------------------------------------------
# Config-driven data loading (Chat 3). No setwd(), no absolute paths: rutas,
# nombres de columnas y contrato de datos salen de config/config.yml +
# config/data_dictionary.csv, resueltos con here::here() relativo al root.
# I/O y validación viven en R/ (fuente única de verdad), cargadas por source()
# con local = TRUE para que caigan en el entorno de render (mismo patrón que el
# chunk nested_cv_helper_functions).
# ----------------------------------------------------------------------------
source(here::here("R", "io.R"),            local = TRUE)
source(here::here("R", "data_contract.R"), local = TRUE)
source(here::here("R", "models.R"),        local = TRUE)   # Chat 4: build_model_specs()
source(here::here("R", "plots.R"),         local = TRUE, encoding = "UTF-8")  # Chat 6: build_*() figuras (labels con ±, ≥)

cfg <- read_config()   # PTB_PROFILE (default = "default")

# Load datasets from config paths (matrix_path / metadata_path / participant_path).
datasets      <- load_dataset(cfg)
complete_data <- datasets$matrix
metadata_long <- datasets$metadata
# participant_data_clean.csv se CARGA-pero-NO-se-usa en el pipeline: participant_path
# está en null en el config, así que load_dataset() no lo lee (datasets$participant = NULL).

# Validate against the data contract: falla claro si falta una columna clave, el
# outcome no es binario, faltan taxa del diccionario, las taxa no están en [0,1], o
# los sample_id/subject_id no casan entre la matriz y la metadata.
validate_input_data(complete_data, metadata_long, cfg)

# Separate data domains BY NAME (role = microbiome en data_dictionary.csv), nunca por
# posición. Corrige el solapamiento del viejo micro_data[,1:99] / clin_data[,98:167],
# donde las columnas 98-99 (index, id — las llaves) caían en AMBOS bloques.
domains    <- split_domains(complete_data, cfg)
micro_data <- domains$microbiome   # index, id + 97 taxa (idéntico a data[,1:99])
clin_data  <- domains$clinical     # index, id + 68 clínicas (idéntico a data[,98:167])
...
# Visualization (build_* en R/plots.R — Chat 6)
build_population_ga_violin(subject_outcomes, n_subjects, n_preterm, prevalence, n_term)
```
...
## Study Cohort Overview and Longitudinal Sampling Design

**Purpose**: Provide comprehensive visualization of study design, participant characteristics, and sampling structure. This figure establishes the nested case-control framework, demonstrates the heterogeneous longitudinal sampling pattern, and shows the distribution of microbiome samples across pregnancy trimesters.

**Key aspects**:
- **Panel A**: Flowchart showing study enrollment and outcome classification
- **Panel B**: Distribution of microbiome samples across gestational trimesters, stratified by birth outcome
- **Panel C**: Swimmer plot illustrating individual-level longitudinal trajectories from first sample to delivery

```{r}
# Color palette (consistent with manuscript)
outcome_colors <- c("Term" = "#619CFF", "Preterm" = "#F8766D")

# Subject-level summary for swimmer plot
swimmer_data <- metadata_long %>%
  group_by(id) %>%
  summarize(
    outcome = ifelse(any(preterm == 1), "Preterm", "Term"),
    ga_delivery = first(sdg_parto),
    first_sample_ga = min(sdg_visita),
    last_sample_ga = max(sdg_visita),
    n_samples = n(),
    .groups = "drop"
  ) %>%
  arrange(outcome, ga_delivery) %>%
  mutate(
    subject_id = factor(id, levels = id),  # Preserve ordering
    subject_number = row_number()
  )

# Individual sample data for swimmer plot points
sample_points <- metadata_long %>%
  left_join(swimmer_data %>% select(id, subject_number, outcome), by = "id") %>%
  select(id, subject_number, outcome, sdg_visita)

# Trimester classification for Panel C
samples_by_trimester <- metadata_long %>%
  mutate(
    outcome = ifelse(preterm == 1, "Preterm", "Term"),
    trimester = case_when(
      sdg_visita < 13 ~ "First\n(< 13 weeks)",
      sdg_visita < 27 ~ "Second\n(13-26 weeks)",
      TRUE ~ "Third\n(≥ 27 weeks)"
    ),
    trimester = factor(trimester, levels = c("First\n(< 13 weeks)", 
                                               "Second\n(13-26 weeks)", 
                                               "Third\n(≥ 27 weeks)"))
  )

# Summary statistics for annotations
n_total_subjects <- nrow(swimmer_data)
n_preterm <- sum(swimmer_data$outcome == "Preterm")
n_term <- sum(swimmer_data$outcome == "Term")
n_total_samples <- nrow(metadata_long)
n_preterm_samples <- sum(metadata_long$preterm == 1)
n_term_samples <- sum(metadata_long$preterm == 0)
prevalence_subject <- round(100 * n_preterm / n_total_subjects, 1)
prevalence_sample <- round(100 * n_preterm_samples / n_total_samples, 1)
mean_samples <- round(mean(swimmer_data$n_samples), 1)
range_samples <- paste(min(swimmer_data$n_samples), "-", max(swimmer_data$n_samples))

cat(
  "\n", paste(rep("=", 70), collapse = ""), "\n",
  "FIGURE 1: STUDY COHORT AND SAMPLING STRUCTURE\n",
  paste(rep("=", 70), collapse = ""), "\n\n",
  
  "Subject-level summary:\n",
  sprintf("  - Total participants: %d\n", n_total_subjects),
  sprintf("  - Preterm births: %d (%.1f%%)\n", n_preterm, prevalence_subject),
  sprintf("  - Term births: %d (%.1f%%)\n", n_term, 100 - prevalence_subject),
  sprintf("  - Samples per subject: mean = %.1f, range = %s\n\n", mean_samples, range_samples),
  
  "Sample-level summary:\n",
  sprintf("  - Total samples: %d\n", n_total_samples),
  sprintf("  - Preterm-associated: %d (%.1f%%)\n", n_preterm_samples, prevalence_sample),
  sprintf("  - Term-associated: %d (%.1f%%)\n", n_term_samples, 100 - prevalence_sample),
  "\n"
)
```
...
```{r figure1_panel_a, fig.width=6, fig.height=4}
# Create flowchart using ggplot with text annotations
# This approach provides publication-quality output without external dependencies

panel_a <- build_figure1_flowchart(
  n_total_subjects, n_preterm, prevalence_subject, n_term,
  n_total_samples, n_preterm_samples, prevalence_sample, n_term_samples,
  mean_samples, range_samples, outcome_colors
)

```

### Panel B: Distribution of Samples Across Pregnancy Trimesters

```{r figure1_panel_c, fig.width=7, fig.height=5}
# Count samples by trimester and outcome
trimester_summary <- samples_by_trimester %>%
  group_by(trimester, outcome) %>%
...
panel_b <- build_figure1_trimester(trimester_summary, outcome_colors)

# Print summary table
cat("\nSample distribution by trimester:\n")
print(
  trimester_summary %>%
    select(trimester, outcome, n, percentage) %>%
    arrange(trimester, desc(outcome)) %>%
    as.data.frame()
)
```

### Panel C: Swimmer Plot (Individual Longitudinal Trajectories)

```{r figure1_panel_b, fig.width=8, fig.height=10}
# Create swimmer plot showing each subject's trajectory
panel_c <- build_figure1_swimmer(
  swimmer_data, sample_points, n_preterm, n_total_subjects, n_term, outcome_colors
)

```

### Combined Figure 1: Study Cohort and Longitudinal Sampling Design

```{r figure1_combined, fig.width=14, fig.height=14, message=FALSE, warning=FALSE}
### Combined Figure 1: Study Cohort and Longitudinal Sampling Design

figure1_combined <- build_figure1_combined(panel_a, panel_b, panel_c)
...
## Decontaminant filter

```{r decontaminant_filter}
# Lista de contaminantes desde config (preprocessing.contaminant_genera), ya no
# hardcodeada. Basada en Salter et al. 2014 / Eisenhofer et al. 2019.
contaminant_genera <- cfg$preprocessing$contaminant_genera

# Filtrar genera
genera_clean <- genera_to_keep[!genera_to_keep %in% contaminant_genera]
# Debería quedar ~40 géneros

# Re-construir tabla de abundancias sin contaminantes
# Apply filtering
microbiome_filtered <- micro_data %>%
  select(index, id, all_of(genera_clean))
```

## Exploratory Analysis: Vaginal Microbiome Composition

**Purpose**: Visualize global structure and variability in vaginal microbiome composition using principal component analysis (PCA) of CLR-transformed genus-level abundance data. This exploratory analysis examines whether pregnancy outcomes (preterm vs. term) show distinct compositional signatures and quantifies the variance captured by the major axes of microbiome variation.

**Key aspects**:
- **Panel A**: PCA scores plot showing sample-level compositional patterns stratified by birth outcome
- **Panel B**: Variance explained by principal components (demonstrates PC1-PC2 capture majority of variance)
- **Panel C** (optional): Biplot with genus loadings showing taxa driving compositional variation

### Data Preparation and CLR Transformation

```{r figure2_data_prep, message=FALSE, warning=FALSE}
# ============================================================================
# IDENTIFY TAXA COLUMNS
# ============================================================================

# Taxa columns are all columns EXCEPT: index, id, shannon_diversity
# Based on your data structure from earlier checks
non_taxa_cols <- c("index", "id", "shannon_diversity")
taxa_cols <- setdiff(names(micro_data), non_taxa_cols)

cat(sprintf("Taxa columns identified: %d genera\n", length(taxa_cols)))

# Verify no missing values in taxa columns (should be filtered already)
taxa_matrix <- micro_data %>% select(all_of(taxa_cols))
n_missing <- sum(is.na(taxa_matrix))
cat(sprintf("Missing values in taxa data: %d\n\n", n_missing))

# ============================================================================
# APPLY CLR TRANSFORMATION (FULL DATASET)
# ============================================================================

cat("Applying Centered Log-Ratio (CLR) transformation...\n")

# Compositional CLR (per SAMPLE) with cmultRepl zero replacement — same method as
# the ML pipeline (see FUNCTION 2). Exploratory full-dataset transform for the PCA.
clr_M    <- as.matrix(micro_data[, taxa_cols]); storage.mode(clr_M) <- "numeric"
clr_R    <- as.matrix(suppressWarnings(zCompositions::cmultRepl(
              clr_M, label = 0, method = "GBM", output = "prop",
              z.delete = FALSE, z.warning = 1)))
clr_logR <- log(clr_R)
micro_clr <- micro_data
micro_clr[, taxa_cols] <- clr_logR - rowMeans(clr_logR)   # each SAMPLE centered (row-sums ~0)

cat("✓ CLR transformation completed\n\n")

# Verify transformation (all CLR values should sum to ~0 for each sample)
clr_matrix <- micro_clr %>% select(all_of(taxa_cols))
row_sums <- rowSums(clr_matrix)
cat(sprintf("Row sums after CLR (should be ~0): mean = %.2e, sd = %.2e\n\n", 
            mean(row_sums), sd(row_sums)))
...
# Extract metadata
metadata_pca <- preterm_data %>%
  mutate(
    outcome = factor(ifelse(preterm == 1, "Preterm", "Term"), 
                    levels = c("Term", "Preterm")),
    outcome_detailed = case_when(
      early_preterm == 1 ~ "Early Preterm (<34 wks)",
      preterm == 1 ~ "Late Preterm (34-36 wks)",
      TRUE ~ "Term (≥37 wks)"
    ),
    outcome_detailed = factor(outcome_detailed, 
                             levels = c("Term (≥37 wks)", 
                                      "Late Preterm (34-36 wks)",
                                      "Early Preterm (<34 wks)"))
  )

# CLR matrix for PCA (samples × genera)
clr_for_pca <- clr_matrix %>% as.matrix()
rownames(clr_for_pca) <- micro_clr$index

cat(sprintf("Data prepared for PCA: %d samples × %d genera\n\n", 
            nrow(clr_for_pca), ncol(clr_for_pca)))

# Summary statistics
cat("Outcome distribution:\n")
print(table(metadata_pca$outcome))
cat("\n")
```

### Perform Principal Component Analysis

```{r figure2_pca_calculation, message=FALSE, warning=FALSE}
cat("Performing PCA on CLR-transformed microbiome profiles...\n")

# Run PCA
# center = TRUE: center variables to mean 0 (important for CLR data)
# scale = FALSE: don't scale (CLR already puts data on comparable scale)
pca_result <- prcomp(clr_for_pca, center = TRUE, scale. = FALSE)
...
cat(sprintf("\nPC1 + PC2 explain: %.2f%% of total variance\n", 
            cumulative_variance[2]))
cat(sprintf("PC1 + PC2 + PC3 explain: %.2f%% of total variance\n\n", 
            cumulative_variance[3]))

# Extract PC scores (sample coordinates)
pca_scores <- as.data.frame(pca_result$x[, 1:5])  # Keep first 5 PCs
# Chat 6 (B): reconstruir el index desde micro_clr$index (mismas filas, mismo ORDEN
# que clr_for_pca -> pca_result$x) preservando su TIPO original. El viejo
# as.integer(rownames(pca_scores)) asumía index numérico y devolvía NA con IDs
# alfanuméricos (p.ej. "S01_V1") de otro laboratorio -> rompía el left_join por
# "index". Es un no-op con IDs enteros (datos reales) -> figuras byte-idénticas.
pca_scores$index <- micro_clr$index

# Merge with metadata
pca_data <- pca_scores %>%
  left_join(metadata_pca, by = "index")

# Extract loadings (genus contributions to PCs)
pca_loadings <- as.data.frame(pca_result$rotation[, 1:3])  # First 3 PCs
pca_loadings$genus <- rownames(pca_loadings)
...
top_pc1 <- pca_loadings %>%
  ...
  head(top_n_loadings) %>%
  mutate(contribution = "PC1")

top_pc2 <- pca_loadings %>%
  arrange(desc(abs(PC2))) %>%
  head(top_n_loadings) %>%
  mutate(contribution = "PC2")

top_loadings <- bind_rows(top_pc1, top_pc2) %>%
  distinct(genus, .keep_all = TRUE)

cat(sprintf("Top %d genera contributing to PC1 and PC2 identified\n\n", 
            nrow(top_loadings)))

# Print top contributors
cat("Top genera contributing to PC1:\n")
top_pc1 %>%
  select(genus, PC1) %>%
...
cat("\nTop genera contributing to PC2:\n")
top_pc2 %>%
  select(genus, PC2) %>%
  mutate(PC2 = round(PC2, 4)) %>%
  head(5) %>%
  print()
...
### Panel A: PCA Scores Plot by Birth Outcome

```{r figure2_panel_a, fig.width=8, fig.height=6}
# Color palette (consistent with Figure 1)
outcome_colors <- c("Term" = "#619CFF", "Preterm" = "#F8766D")

# Calculate 95% confidence ellipses
# Using stat_ellipse with type = "norm" for normal distribution assumption

panel_a <- build_figure2_pca_scatter(pca_data, variance_explained, outcome_colors)

print(panel_a)

# Calculate summary statistics for overlap
cat("\nPCA score statistics by outcome:\n")
...
```{r figure2_panel_b, fig.width=7, fig.height=5}
panel_b <- build_figure2_scree(variance_explained, cumulative_variance)

print(panel_b)
```

### Panel C: Biplot with Genus Loadings (Optional)

```{r figure2_panel_c, fig.width=9, fig.height=7}
panel_c <- build_figure2_biplot(pca_data, top_loadings, variance_explained, outcome_colors)

panel_c
```

**Technical notes:** 
- Principal component analysis was performed on centered log-ratio (CLR)-transformed genus-level relative abundances to account for the compositional nature of microbiome data (Gloor et al., 2017, doi:10.3389/fmicb.2017.02224). 
- Compositional zeros were replaced with unsupervised Bayesian-multiplicative estimates (zCompositions::cmultRepl) before the per-sample log-ratio transformation.
- PCA was performed on the full dataset (n=110 samples) using all [N] genera that passed quality filtering (≥5% sample prevalence). 
- Confidence ellipses assume multivariate normality and are shown for visualization purposes; formal statistical comparison was performed using PERMANOVA on Aitchison distances (Euclidean distance on CLR-transformed data).
...
# Chat 4: desde config (approaches.approach1_dream) — ya no hardcodeado.
# sdg_visita = gestational age at visit (temporal); edad_cronologicamujer = maternal age.
approach1_vars <- cfg$approaches$approach1_dream
...
# ============================================================================
# Chat 4: el tribble se reconstruye 100% desde config (approaches.approach2_literature).
# core_vars + candidates, cada uno {evidence, source}, EN EL ORDEN del config (load-bearing:
# rompe empates en head(target_n) del STEP 4). Copia verbatim del tribble original; la
# LOGICA de filtrado/ranking abajo NO cambia — solo los insumos salen de config.
a2_cfg <- cfg$approaches$approach2_literature
a2_entries_to_rows <- function(entries) {
  tibble(
    variable          = names(entries),
    evidence_strength = unname(vapply(entries, function(e) e$evidence, character(1))),
    source            = unname(vapply(entries, function(e) e$source,   character(1)))
  )
}
approach2_candidates <- bind_rows(
  a2_entries_to_rows(a2_cfg$core_vars),   # sdg_visita, edad_cronologicamujer (siempre presentes)
  a2_entries_to_rows(a2_cfg$candidates)   # 15 candidatos de literatura, en orden de config
)
...
# Filter to variables with ≥ completeness_min subject completeness (config; 0.80 -> 80%)
valid_vars_approach2 <- completeness_approach2 %>%
  filter(pct_subjects_complete >= a2_cfg$rules$completeness_min * 100) %>%
  pull(variable)
...
collinear_pairs_approach2 <- detect_collinear_vars(
  ...
  threshold = a2_cfg$rules$collinearity_cutoff   # config (0.95)
)
...
# Ensure core variables are included (from config; se agregan por nombre)
core_vars <- names(a2_cfg$core_vars)
core_vars_present <- intersect(core_vars, valid_vars_approach2)

# Select remaining variables up to target_n (config; 10)
remaining_slots <- a2_cfg$rules$target_n - length(core_vars_present)
...
- Univariate screening threshold: p < 0.30 (liberal for small n, hypothesis-generating)
- Candidate cap before collinearity removal: top 15 by p-value
- Maximum features: 10 (to prevent overfitting)
- No mandatory inclusions: every variable competes on its per-fold univariate p-value
...
# Abs-count file existence check via config path (no absolute paths). La carga y el
# split por nombre los hace io.R::load_abs_matrix(cfg) en run_nested_cv; aquí solo se
# verifica la disponibilidad para esta sección demostrativa.
abs_file <- here::here(cfg$data$abs_matrix_path)
...
# Chat 4: las specs de los modelos ACTIVOS se construyen desde config$models
# (rf_base = ranger, glmnet_base = glmnet) via build_model_specs() en R/models.R —
# ya no hardcodeadas. Los 3 specs desactivados (xgb_base, tree_base, rf_pca) viven
# ahora en config$models_future (reservados para dataset ampliado; ver roadmap en
# CLAUDE.md). build_model_specs() IGNORA models_future -> se mantienen las 12
# combinaciones (2 modelos x 3 approaches x 2 microbioma).
models_spec <- build_model_specs(cfg)

cat(
  "Models configured (activos, desde config$models): ", length(models_spec), "\n\n",
  ...
  "Reservados (config$models_future, NO activos): xgb_base, tree_base, rf_pca\n",
  "  -> reactivar = mover a config$models + revalidar convergencia (ver CLAUDE.md)\n"
)
```
...
# ========================================================================
# Chat 4: solo los modelos activos (config$models). Los specs xgb_base / tree_base /
# rf_pca ya NO se extraen: estan desactivados en config$models_future (ver CLAUDE.md).
# El orquestador (run_nested_cv) itera directamente sobre models_spec (config-driven),
# de modo que "activar" un modelo = moverlo a config$models sin tocar este chunk.
...
- Replaces zeros with cmultRepl (Bayesian-multiplicative) estimates, then centers each SAMPLE by the geometric mean of its own taxa (per-sample CLR)
...
# ----------------------------------------------------------------------------
# Extraidas a R/ en el Chat 2. Fuente unica de verdad = los archivos en R/;
# aqui solo se cargan por source() con here::here() (rutas relativas al root
# del proyecto, sin setwd ni rutas absolutas):
#   R/clr.R                fit_clr_zerorepl(), apply_clr_transform()
#   R/threshold.R          optimize_threshold_cv()
#   R/feature_selection.R  run_ancombc_on_fold()
#   R/nested_cv.R          detect_prob_col(), train_with_nested_cv()
# El objeto clr_zero_levels se computa una vez en el chunk run_nested_cv y se
# pasa por argumento a train_with_nested_cv() (ya no es un default global).
# ============================================================================
# local = TRUE: definir las funciones en el ENTORNO del chunk (el mismo donde viven
# genera_clean, subject_labels, micro_genus_full y los helpers univariados que estas
# funciones leen del entorno). Con el default local = FALSE, source() las definiria
# en .GlobalEnv, que difiere del envir de render() -> lookups globales rotos.
source(here::here("R", "clr.R"),               local = TRUE)
source(here::here("R", "threshold.R"),         local = TRUE)
source(here::here("R", "feature_selection.R"), local = TRUE)
source(here::here("R", "nested_cv.R"),         local = TRUE)

cat("Motor de nested CV cargado desde R/ (clr, threshold, feature_selection, nested_cv)\n")
```
...
# ============================================================================
# Configuration: 2 models × 3 approaches × 2 microbiomes = 12 combinations
...
# Chat 4: modelos activos desde config$models (via models_spec = build_model_specs(cfg)).
# Iterar sobre names(models_spec) preserva el orden del config y hace que "activar" un
# modelo futuro = moverlo a config$models (ver roadmap CLAUDE.md), sin tocar este chunk.
# Ningun modelo activo usa PCA (rf_pca esta en models_future) -> pca = FALSE.
models_to_run <- lapply(names(models_spec), function(nm) {
  list(name = nm, spec = models_spec[[nm]], pca = FALSE)
})
...
# Chat 4: nombres desde config$microbiome_options. use_ancom = TRUE solo para "ANCOM_Taxa"
# (los taxa se seleccionan por fold); "Full_Microbiome" usa todos los taxa tal cual.
microbiome_config <- lapply(cfg$microbiome_options, function(nm) {
  list(
    name = nm,
    data = micro_genus_full,               # Base data (taxa selected per fold if ANCOM)
    use_ancom = identical(nm, "ANCOM_Taxa") # Flag to trigger ANCOM analysis
  )
})
...
cat(
  ...
  "✓ Microbiome options: ", length(microbiome_config), "\n\n",
  sep = ""
)

# ============================================================================
# CLR ZERO-REPLACEMENT LEVELS (learned once, unsupervised — see FUNCTION 2)
# ============================================================================
# cmultRepl replacement is outcome-blind; per-fold refitting is infeasible on this
# sparsity (all-zero taxa within some training splits). apply_clr_transform() reads
# clr_zero_levels by default and applies stateless per-sample CLR within each fold.
clr_taxa_all   <- setdiff(names(micro_genus_full), c("index", "id", "shannon_diversity"))
clr_zero_levels <- fit_clr_zerorepl(micro_genus_full, clr_taxa_all)
cat(sprintf("CLR zero-replacement learned for %d taxa (min=%.2e, median=%.2e, max=%.2e)\n\n",
            length(clr_taxa_all), min(clr_zero_levels), median(clr_zero_levels), max(clr_zero_levels)))

# ============================================================================
# ABS-COUNT MATRIX FOR ANCOM-BC2 (loaded once, by name, from config)
# ============================================================================
# Reemplaza la lectura por-fold de una ruta absoluta hardcodeada. io.R separa la
# matriz en otu (taxa) y meta (subject_id + clínicas) por NOMBRE. Se pasa a
# train_with_nested_cv() junto con genera_clean (solo la rama ANCOM_Taxa la usa).
abs_data <- load_abs_matrix(cfg)
cat(sprintf("Abs-count matrix loaded for ANCOM-BC2: %d samples × %d taxa\n\n",
            nrow(abs_data$otu), ncol(abs_data$otu)))
...
successful_models <- 0
failed_models <- 0
total_combinations <- length(models_to_run) * length(approaches_config) * length(microbiome_config)
...
cat(
  ...
  sep = ""
      ...
      result <- train_with_nested_cv(
        ...
        cv_folds = cv_folds,
        clr_zero_levels = clr_zero_levels,   # threaded in (was a global default in apply_clr_transform)
        genera_clean = genera_clean,         # Chat 3: era global (chunk decontaminant_filter)
        abs_data = abs_data,                 # Chat 3: era ruta absoluta + indexado posicional
        use_pca = model$pca
      )
...
if(length(approach3_selection_data) > 0) {
  ...
  p1 <- build_approach3_selection_freq(var_frequency_approach3)
  ...
  p2 <- build_approach3_pvalue_stability(var_frequency_approach3)
  ...
  p3 <- build_approach3_selection_heatmap(selection_matrix, var_order)
...
2. **Fold-specific selection** (1-2 folds) expected due to:
   - Statistical power limitations (with p<0.30 threshold, genuine associations may miss significance in some folds)
...
if(length(ancom_selection_data) > 0) {
  ...
  # Visualization
  p_taxa <- build_ancom_taxa_freq(taxa_frequency)
...
p_roc <- build_roc_curves(roc_summary, labels_ordered, colors)
...
p_pr <- build_pr_curves(pr_summary, labels_ordered, colors, prevalence)
...
## Feature Importance and Selection Stability Analysis (CORRECTED)

**Purpose**: Characterize the contribution and stability of clinical and microbiome features in the best-performing Random Forest model using Gini impurity-based importance complemented by feature selection stability analysis.

### Data Preparation and Model Extraction

```{r figure5_corrected_prep, message=FALSE, warning=FALSE}
library(tidyverse)
library(patchwork)
library(workflows)
...
cat(
  "\n", paste(rep("=", 70), collapse = ""), "\n",
  "FIGURE 5: FEATURE IMPORTANCE AND STABILITY ANALYSIS\n",
  paste(rep("=", 70), collapse = ""), "\n\n"
)
...
rf_models <- map_dfr(1:length(cv_results_all), function(i) {
  res <- cv_results_all[[i]]
  if(!is.null(res) && res$model_name == "rf_base") {
    tibble(
      idx = i,
      model_name = res$model_name,
      approach = res$approach,
...
cat(sprintf("Best-Performing Random Forest Model:\n"))
...
if(is.null(best_rf$final_model)) {
  stop("ERROR: No final model stored. Cannot compute feature importance.")
}
```

### Extract Gini-Based Feature Importance

```{r figure5_gini_importance, message=FALSE, warning=FALSE}
cat("╔════════════════════════════════════════════════════════════════════╗\n")
cat("║  EXTRACTING FEATURE IMPORTANCE (GINI IMPURITY)                    ║\n")
cat("╚════════════════════════════════════════════════════════════════════╝\n\n")

cat("Method: Gini impurity-based importance\n")
cat("Source: Variable importance from trained Random Forest\n")
cat("Note: Mean decrease in Gini impurity across all trees\n\n")

# Extract fitted RF model
rf_fit <- extract_fit_parsnip(best_rf$final_model)

# Verify importance is available
if(is.null(rf_fit$fit$variable.importance)) {
  stop("ERROR: Model does not have variable importance. Check model specification.")
}

# Extract importance scores
gini_importance <- tibble(
  Variable = names(rf_fit$fit$variable.importance),
  Importance = rf_fit$fit$variable.importance
) %>%
  arrange(desc(Importance))

cat(sprintf("✓ Extracted importance for %d features\n\n", nrow(gini_importance)))

# ============================================================================
# CLASSIFY FEATURES BY TYPE
# ============================================================================

# Comprehensive list of clinical variables
...
known_genera <- c(
  ...
  "Gordonia", "Peptostreptococcus", "Kocuria", "Agrobacterium",
  "Variovorax", "Bradyrhizobium"
)

# Classify and clean feature names
importance_classified <- gini_importance %>%
  mutate(
    feature_type = case_when(
      grepl("shannon", Variable, ignore.case = TRUE) ~ "Diversity",
      Variable %in% clinical_vars_comprehensive ~ "Clinical",
      gsub("_[^_]+$", "", Variable) %in% clinical_vars_comprehensive ~ "Clinical",
      sapply(Variable, function(f) {
        any(sapply(clinical_vars_comprehensive, function(cv) startsWith(f, cv)))
      }) ~ "Clinical",
      grepl("^CLR_", Variable) ~ "Microbiome",
      grepl("^Genus_", Variable) ~ "Microbiome",
      sapply(Variable, function(f) {
        any(sapply(known_genera, function(g) grepl(g, f, ignore.case = TRUE)))
      }) ~ "Microbiome",
      TRUE ~ "Microbiome"
    ),
    Variable_clean = case_when(
      ...
      Variable == "sdg_visita" ~ "Gestational Age at Sample",
      Variable == "shannon_diversity" ~ "Shannon Diversity",
      ...
      Variable == "nivel_academico" ~ "Education Level",
      Variable == "low_education" ~ "Low Education",
      Variable == "unmarried" ~ "Unmarried Status",
      Variable == "ses_risk_score" ~ "SES Risk Score",
      grepl("^CLR_", Variable) ~ gsub("CLR_", "", Variable),
      grepl("^Genus_", Variable) ~ gsub("Genus_", "", Variable),
      grepl("imc_pregest_categ", Variable) ~ gsub("imc_pregest_categ_", "BMI Cat: ", Variable),
      grepl("age_risk_category", Variable) ~ gsub("age_risk_category_", "Age Cat: ", Variable),
      grepl("sex_baby", Variable) ~ gsub("sex_baby_", "Baby Sex: ", Variable),
      TRUE ~ Variable
    ),
    Variable_clean = str_trunc(Variable_clean, width = 40)
  ) %>%
  arrange(desc(Importance))

# Summary statistics
cat("Feature type distribution (all features):\n")
print(table(importance_classified$feature_type))
cat("\n")

cat("Feature type distribution (top 25):\n")
print(table(importance_classified$feature_type[1:25]))
cat("\n")
```

### Panel A: Feature Importance (Top 20)

```{r figure5_panel_a, fig.width=10, fig.height=8}
# Top features: 20 for the plot, 25 for the text/summary tables below
# (top25_importance is also used by the figure5_summary_stats chunk).
top20_importance <- importance_classified %>% head(20)
top25_importance <- importance_classified %>% head(25)

# Create importance plot
panel_a <- build_figure5_importance(top20_importance)

print(panel_a)

# Print top 10 for text
cat("\n=== TOP 10 MOST IMPORTANT FEATURES ===\n\n")
top10_table <- top25_importance %>%
  head(10) %>%
  select(Variable, Variable_clean, feature_type, Importance) %>%
  mutate(
    Rank = 1:n(),
    Importance = round(Importance, 3)
  ) %>%
  select(Rank, Feature = Variable_clean, Type = feature_type, Importance)

print(top10_table)
cat("\n")
```

### Panel B: Clinical Feature Selection Stability

```{r figure5_panel_b_improved, fig.width=10, fig.height=8}
cat("╔════════════════════════════════════════════════════════════════════╗\n")
cat("║  CLINICAL FEATURE SELECTION STABILITY (APPROACH 3)                ║\n")
cat("╚════════════════════════════════════════════════════════════════════╝\n\n")

if(is.null(best_rf$selected_variables)) {
  cat("⚠️  WARNING: No selected_variables found in best model.\n")
  panel_b <- ggplot() + theme_void()
  
} else {
  
  selected_vars <- best_rf$selected_variables
  
  cat(sprintf("Selected variables data: %d rows across %d folds\n", 
              nrow(selected_vars), length(unique(selected_vars$fold))))
  
  # Calculate selection frequency
  var_frequency <- selected_vars %>%
    group_by(variable) %>%
    summarize(
      n_folds_selected = n_distinct(fold),
      total_folds = 5,
  ...
  # ========================================================================
  # MODIFIED: Show ALL clinical features from Approach 3
  # Not just those in top 25 importance ranking
  # ========================================================================
  
  cat(sprintf("Total unique variables selected across folds: %d\n", nrow(var_frequency)))
  
  panel_b <- build_figure5_stability(var_frequency)
...
### Combined Figure 5: Feature Importance and Selection Stability

```{r figure5_combined, fig.width=16, fig.height=8, message=FALSE, warning=FALSE}
# Combine panels side-by-side
figure5_combined <- build_figure5_combined(panel_a, panel_b, best_rf_info)
...
```{r figure5_summary_stats}
cat("╔════════════════════════════════════════════════════════════════════╗\n")
cat("║  SUMMARY STATISTICS FOR MANUSCRIPT TEXT                            ║\n")
cat("╚════════════════════════════════════════════════════════════════════╝\n\n")

# Feature type breakdown in top 25
feature_type_counts <- table(top25_importance$feature_type)

cat("Feature composition in top 25:\n")
for(type in names(feature_type_counts)) {
  cat(sprintf("  - %s: %d (%.1f%%)\n", 
              type,
              feature_type_counts[type],
              100 * feature_type_counts[type] / 25))
}

cat("\nTop 5 features overall:\n")
for(i in 1:5) {
  cat(sprintf("  %d. %s [%s] (importance = %.3f)\n",
              i,
              top25_importance$Variable_clean[i],
              top25_importance$feature_type[i],
              top25_importance$Importance[i]))
}

cat("\nTop 3 clinical features:\n")
top_clinical <- top25_importance %>%
  filter(feature_type == "Clinical") %>%
  head(3)
for(i in 1:nrow(top_clinical)) {
  cat(sprintf("  %d. %s (importance = %.3f)\n",
              i,
              top_clinical$Variable_clean[i],
              top_clinical$Importance[i]))
}

cat("\nTop 3 microbiome features:\n")
top_microbiome <- top25_importance %>%
  filter(feature_type == "Microbiome") %>%
  head(3)
if(nrow(top_microbiome) > 0) {
  for(i in 1:nrow(top_microbiome)) {
    cat(sprintf("  %d. %s (importance = %.3f)\n",
                i,
                top_microbiome$Variable_clean[i],
                top_microbiome$Importance[i]))
  }
} else {
  cat("  (No microbiome features in top 3)\n")
}
...
**Figure 5. Feature Importance and Selection Stability in Best-Performing Random Forest Model.**

**(A)** Variable importance for the top 25 predictive features based on mean decrease in Gini impurity. Importance reflects how much each feature contributes to reducing impurity across all decision trees in the Random Forest ensemble, with higher values indicating greater discriminatory power between preterm and term births. Features are colored by type: clinical variables (red, n=[X]), microbial genera (blue, n=[Y]), and diversity metrics (green, n=[Z]). Anthropometric features—particularly BMI-related variables (BMI at visit, pre-pregnancy weight, extreme BMI status, underweight indicators)—dominate the upper importance ranks, consistent with population-specific maternal health patterns in this Mexican cohort. Microbiome features appear throughout the importance hierarchy rather than clustered at low ranks, with genera such as [top 3 genera] demonstrating importance comparable to several clinical risk factors, supporting the complementary predictive value of vaginal microbiome profiling alongside traditional clinical assessment.

**(B)** Selection stability of all clinical features identified through data-driven univariate screening (Approach 3) across 5 cross-validation folds. Univariate screening was performed independently within each training fold using a liberal p<0.30 threshold, with top 10 features selected per fold after collinearity resolution. Bar height indicates selection frequency (number of folds selecting each feature, out of 5), while bar color represents the strength of univariate association with preterm birth (mean p-value across selected folds): dark green (p<0.01, very strong), light green (p<0.05, strong), yellow (p<0.10, moderate), orange (p<0.20, weak), and red (p≥0.20, very weak). All 20 unique clinical variables selected across the 5 folds are shown. No features were selected in all 5 folds (very stable), indicating substantial inter-fold variability in univariate associations, likely reflecting the modest sample size (n=43 subjects) and borderline statistical power for individual predictors. Features selected in 4 folds with strong univariate associations (e.g., Extreme BMI: 4/5 folds, p=0.0003) represent the most robust clinical predictors. Divergent patterns emerge: some features show strong univariate associations (p<0.05) but low selection frequency (1-2 folds), suggesting fold-specific patterns or collinearity-driven exclusion, while others are consistently selected (4 folds) despite moderate univariate associations (p~0.10), potentially reflecting importance in multivariate context. Microbiome features are not shown as the best-performing model used the full microbiome approach (all 59 genera included in all folds) rather than fold-specific selection.

**Technical notes:**
- Feature importance calculated from final Random Forest model trained on complete dataset (n=110 samples, 43 subjects, 500 trees, mtry=4, min.node.size=10).
- Importance metric: Mean decrease in Gini impurity, averaged across all trees in ensemble.
- Clinical feature selection performed using data-driven univariate screening (liberal p<0.30 threshold) independently in each cross-validation fold.
- Model configuration: Approach 3 (data-driven features) + Full Microbiome (59 genera).
- Model performance: AUROC = [0.849 ± 0.130], PRAUC = [0.571 ± 0.208] from 5-fold subject-level nested cross-validation.

**Methodological note on importance metric:**
Gini impurity-based importance was used rather than permutation-based importance due to computational compatibility with the tidymodels workflow framework employed in this study. While permutation importance offers theoretical advantages (unbiased by class imbalance, more interpretable as performance degradation), Gini importance remains a widely accepted metric in Random Forest applications and provides convergent evidence when combined with selection stability analysis (Panel B). The consistency of feature selection across independent folds (Panel B) supports the robustness of identified clinical predictors beyond single-model importance estimates.
...
# AUROC comparison
p_auroc <- build_performance_auroc(plot_data_all)
...
p_sens_spec <- build_performance_sens_spec(summary_data)
