...
panel_b <- ggplot(trimester_summary, aes(x = trimester, y = n, fill = outcome)) +
...
panel_a <- ggplot(pca_data, aes(x = PC1, y = PC2, color = outcome, fill = outcome)) +
...
panel_b <- ggplot(variance_df, aes(x = PC)) +
...
figure5_combined <- (panel_a | panel_b) +
  plot_annotation(
    tag_levels = 'A',
    title = sprintf(
